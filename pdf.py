...
# Default config
DEFAULT_CONFIG = {"default workspace": "",
                  "default destination": "",
                  "default quality": ""}
...
# Converts the image at [address] to a jpg if [convert]. Regardless, the image
# is then compressed to quality [q]. It is stored in the address generated by
# [assignTmpName(address)], and the address is returned.
def compressWithConversion(address, q, convert):
    with Image.open(address) as image:
        ...
        if convert:
            ...
            output.paste(image, image)
        else:
            output = image
        if q != -1:
            output.save(outputName, quality=q)
        else:
            output.save(outputName, quality=95)
        return outputName
...
def preprocessImages(q):
    for i in range(len(data.convertNames)):
        ...
        # [convert] if the file is not a jpg but convertable to jpg.
        convert = extension != ".jpg" and extension in SUPPORTED_FORMATS
        # File needs to be converted or compressed.
        if convert or q != -1:
            processedName = compressWithConversion(address, q, convert)
            data.convertNames[i] = processedName
            data.createdFiles.add(processedName)
    ...
    try:
        for key in fields:
            ...
            if value != "":  # only update if user selects a path
                # Check that default quality is valid:
                if key == "default quality":
                    qInt = int(value)
                    if not (1 <= qInt <= 95):
                        raise ValueError
        ...
        displayError(QUALITY_ERROR_TITLE, QUALITY_ERROR, topLevel)
    except Exception:
...
def changeDefaults():
    ...
    currentDest = data.config.get("default quality")
    text3Ptr = stringVarOf(currentDest)

    fields = {"default workspace": text1Ptr,
              "default destination": text2Ptr,
              "default quality": text3Ptr}

    tl = Toplevel()
    defaultsWindow = defaultFrameNoResize(
        tl, "530x210", "Change default settings")
    rowNumber = 0
    ...
    # Third row (quality label, field, button)
    label = defaultLabel("Default Image Quality:", defaultsWindow)
    label.grid(row=rowNumber, column=0, pady=10, padx=5, columnspan=2)

    field = defaultField(text3Ptr, defaultsWindow)
    field.grid(row=rowNumber, column=2, pady=10, padx=5, columnspan=2)

    button = defaultButton("Reset", lambda: resetConfigField(
        text3Ptr, "default quality", "", defaultsWindow
    ), defaultsWindow)
    button.grid(row=rowNumber, column=5, pady=4, padx=1)
...
defaultQuality = data.config.get("default quality")
if defaultQuality == "":
    qualityVar = stringVarOf(QUALITY_FIELD_DEFAULT)
else:
    qualityVar = stringVarOf(defaultQuality)
